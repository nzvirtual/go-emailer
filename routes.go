...
func SetupRoutes(engine *gin.Engine) {
	engine.POST("/send/registration", func(c *gin.Context) {
		data := SendRegistrationDTO{}
		if err := c.ShouldBindJSON(&data); err != nil {
			log.Category("api/send/registration").Error("Invalid email response received " + err.Error())
		...
		if err := tmpl.Execute(&body, data); err != nil {
			log.Category("api/send/registration").Error("Failed to execute template: " + err.Error())
			c.JSON(http.StatusBadRequest, gin.H{"message": "Bad Request"})
			return
		}
